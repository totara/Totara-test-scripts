<?php
// xThis file is part of Moodle - http://moodle.org/
//
...
class moodle_Sniffs_Files_BoilerplateCommentSniff implements PHP_CodeSniffer_Sniff {
    protected static $commentmoodle = array(
    ...
    protected static $commenttotara = array(
        "/*",
    ...
    public function process(PHP_CodeSniffer_File $file, $stackptr) {
        ...
        if (!strcmp(trim(self::$commenttotara[1]), trim($tokens[2]['content']))) {
            $comment = self::$commenttotara;
        } else {
            $comment = self::$commentmoodle;
        }
        foreach ($comment as $lineindex => $line) {
